...
$ID = $PDO->query("SELECT graph FROM templogger.Thresholds WHERE col = 1;");
$ID1 = array();
foreach($ID->fetchAll(PDO::FETCH_ASSOC) as $row) 
{$ID1 = floatval($row['graph']);}
...
$temp = array(
	'label' => $warn1.$str."<font color=\"#0000FF\"<small><small>".$DIS."</small></small></font>", // only way I found of making font bigger
	'data' => $data4
);
